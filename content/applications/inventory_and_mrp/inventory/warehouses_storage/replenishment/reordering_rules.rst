...
lead times. It displays needs **only** when it is time to reorder items, thanks for the
...
- :guilabel:`Bill of Materials`: When the selected :guilabel:`Preferred Route` is
  ...
  |BoM| in the :guilabel:`Bill of Materials` column in the replenishment report prioritizes it when
  creating draft manufacturing orders when the reordering rules need to be triggered.
...
- :guilabel:`Procurement Group`: is a way to group related purchase or manufacturing orders that
  are tied to fulfilling a specific demand, like a sales order or a project. It helps organize and
  track which orders are linked to a particular demand.
  ...
  .. spoiler:: How can you set the Procurement Group, Vendor, and Preferred Route fields on the
     replenishment report to generate a single RFQ for five different products in sales order SO35,
     given they share the same vendor, Azure Interior, and ensure other demands for these products
     ...
     #. Set the :guilabel:`Vendor` to `Azure Interior` to ensure the |RFQ| is created for the same
        supplier.
     ...
     After placing the order, remove `SO35` from the :guilabel:`Procurement Group` field of the five
     products' reordering rules. This ensures future demands for these products are managed
     separately and assigned to different |RFQs| (the usual behavior).
...
.. tip::
   It is perfectly fine to skip setting up lead times, understanding just in time logic, and rely on
   immediate demand. By keeping sales order delivery dates as their creation date, everything is
   treated as needed right away, and reordering rules are triggered immediately.
...
.. example::
   ...
   .. figure:: report/zero-forecast.png
      :alt: Forecast and To Order quantites is zero.
...
*Visibility days* enables the ability to determine if additional quantities should be added to the
planned replenishment. Odoo checks:
